...
class TaskControllerTest extends TestCase
{
    ...
    public function test_valid_user_can_update_task(): void {
        ...
        $this->updateTaskAs(['title' => 'viewer title'], $viewer, $project, $objective, $task)
            ->assertForbidden()
            ->assertJson(['success' => false, 'message' => 'This action is unauthorized, TKPUTK']);
        
        $this->assertDatabaseMissing('tasks', ['title' => 'viewer title']);

        $this->updateTaskAs(['title' => 'user title'], $user, $project, $objective, $task)
            ->assertOk()
            ->assertJson(['success' => true, 'message' => 'Task updated successfully']);

        $this->assertDatabaseHas('tasks', ['title' => 'user title']);

        $this->updateTaskAs(['title' => 'owner title'],$owner, $project, $objective, $task)
            ->assertOk()
            ->assertJson(['success' => true, 'message' => 'Task updated successfully']);
    ...
    public function test_task_status_can_be_updated(): void {
        ...
        $this->actingAs($stranger)
            ->patchJson(
                route('projects.objectives.tasks.status', [$objective, $task]),
                ['status' => TaskStatus::Assigned->value])
            ->assertForbidden()
            ->assertJson(['success' => false, 'message' => 'This action is unauthorized, TKPUSTK']);

        $this->assertDatabaseMissing('tasks', ['status' => TaskStatus::Assigned->value]);
        ...
        $this->actingAs($owner)
            ...
            ->assertOk()
            ->assertJson(['success' => true, 'message' => 'Task status updated successfully']);
        $this->assertDatabaseHas('tasks', ['status' => TaskStatus::InProgress->value]);
         ...
         $this->actingAs($user)
            ...
            ->assertForbidden()
            ->assertJson(['success' => false, 'message' => 'This action is unauthorized, TKPUSTK']);
        $this->assertDatabaseMissing('tasks', ['status' => TaskStatus::Canceled->value]);
    }
    ...
    public function test_user_only_can_update_his_own_tasks(): void {
        ...
        $this->actingAs($userOwner)
            ...
            ->assertOk()
            ->assertJson(['success' => true, 'message' => 'Task status updated successfully']);
        $this->assertDatabaseHas('tasks', ['status' => TaskStatus::Completed]);
    }
}
